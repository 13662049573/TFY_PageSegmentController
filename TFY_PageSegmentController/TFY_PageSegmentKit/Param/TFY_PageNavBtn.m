...
@interface TFY_PageNavBtn ()
@property(nonatomic,assign)CGSize minSize;
@end

@implementation TFY_PageNavBtn
...
@implementation TFY_PageNavBtn (Badge)
- (void)showBadgeWithTopMagin:(NSDictionary*)info
{
    if (!self.badge) {
        CGRect frame = CGRectMake(self.maxSize.width+self.param.menuCellMargin/2, self.param.menuCellPadding/2-pointWidth, pointWidth, pointWidth);
    ...
    if (self.param.customRedView) {
        self.param.customRedView(self.badge,info);
    }
}
