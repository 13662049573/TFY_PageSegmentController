//
//  TFY_DynamicOperatVC.m
//  TFY_PageSegmentKit
//
//  Created by 田风有 on 2021/1/16.
//

#import "TFY_DynamicOperatVC.h"
#import "TFY_TopSuspensionVC.h"
@interface TFY_DynamicOperatVC ()

@end

@implementation TFY_DynamicOperatVC

- (void)viewDidLoad {
    [super viewDidLoad];
    //标题数组
    NSArray *data = @[@"热门",@"男装",@"美妆",@"手机",@"食品",@"电器",@"鞋包",@"百货",@"女装",@"汽车",@"电脑"];
    NSMutableArray *controllerArr = [NSMutableArray new];
    for (int i = 0; i<data.count; i++) {
        TFY_TopSuspensionVC *vc = [TFY_TopSuspensionVC new];
        vc.page = i;
        [controllerArr addObject:vc];
    }
    TFY_PageParam *param = PageParam()
    .titleArrSet(data)
    .menuDefaultIndexSet(3)
    .controllersSet(controllerArr);  //必须使用此属性
    self.param = param;
    
    UIButton *titleDelete = [self getBtnTitle:@"删除"];
    [titleDelete addTarget:self action:@selector(deleteAcction) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *titleDeleteBarItem = [[UIBarButtonItem alloc] initWithCustomView:titleDelete];
    self.navigationItem.rightBarButtonItem = titleDeleteBarItem;

    UIButton *titleAdd = [self getBtnTitle:@"增加"];
    [titleAdd addTarget:self action:@selector(addAcction) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *titleAddBarItem = [[UIBarButtonItem alloc] initWithCustomView:titleAdd];
    
    
    UIButton *back = [self getBtnTitle:@"返回"];
    [back addTarget:self action:@selector(backAction) forControlEvents:UIControlEventTouchUpInside];
    
    self.navigationItem.rightBarButtonItem = titleAddBarItem;
    self.navigationItem.leftBarButtonItems = @[[[UIBarButtonItem alloc] initWithCustomView:back],titleDeleteBarItem];
}

- (void)deleteAcction{
    //下标删除
   BOOL succcess1 =  [self deleteMenuTitleIndex:@(3)];
    
    //标题删除
   BOOL succcess2 =  [self deleteMenuTitleIndex:@"食品"];
    
    //删除数组
   BOOL succcess3 =  [self deleteMenuTitleIndexArr:@[@"食品",@(1)]];
    
   NSLog(@"%d %d %d",succcess1,succcess2,succcess3);
}

- (void)addAcction{
    
    TFY_TopSuspensionVC *AddVC = [TFY_TopSuspensionVC new];
    AddVC.page = 20;
    //增加文本标题
    BOOL succcess1 =  [self addMenuTitleWithObject:[TFY_PageTitleModel initWithIndex:0 controller:AddVC title:@"增加的标题"]];
    
    TFY_TopSuspensionVC *AddVC2 = [TFY_TopSuspensionVC new];
    AddVC2.page = 30;
    //增加字典标题
    BOOL succcess2 =  [self addMenuTitleWithObject:[TFY_PageTitleModel initWithIndex:3 controller:AddVC2 titleInfo:@{@"name":@"字典标题",@"titleColor":[UIColor orangeColor]}]];
    
    //增加数组
    BOOL succcess3 =  [self addMenuTitleWithObjectArr:@[
     [TFY_PageTitleModel initWithIndex:5 controller:[TFY_TopSuspensionVC new] title:@"增加的标题5"],
     [TFY_PageTitleModel initWithIndex:6 controller:[TFY_TopSuspensionVC new] title:@"增加的标题6"],
    ]];
    NSLog(@"%d %d %d",succcess1,succcess2,succcess3);
}

-(void)backAction{
    [self.navigationController popViewControllerAnimated:YES];
}

- (UIButton*)getBtnTitle:(NSString*)title{
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    [btn setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
    [btn setTitle:title forState:UIControlStateNormal];
    [btn sizeToFit];
    return btn;
}


@end
